...
nComb = nValue ^ nPara
realParas = matrix(NA, nValue^nPara, nPara)
realParas[,1] = rep(seq(0.01, 0.03, length.out = 3), nValue^(nPara - 1)) # phi
realParas[,2] = rep(c(0.001, 0.1, 0.2), each = nValue)
nRep = 5
simIdx = t(matrix(1 : (nComb * nRep), nRep, nComb))
dir.create("genData/simulation/monteSteep")
save("realParas", "nComb", "nValue", "nPara", "paraNames", "simIdx", "nRep", file = "genData/simulation/monteSteep/realParas.RData")