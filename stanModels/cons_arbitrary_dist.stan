...
model {
  ...
  for(tIdx in 1 : N){
    ...
    for(i in 1 : (nTimePoints[tIdx]-1)){
      target += log(1 - exp(waitLogProb[i+1,tIdx])) + sum(waitLogProb[1:i, tIdx]) + normal_lpdf(i * stepDuration | timeWaited[tIdx], sigma);
    } 
    target += sum(waitLogProb[1:nTimePoints[tIdx], tIdx]) + normal_lpdf(nTimePoints[tIdx] * stepDuration | timeWaited[tIdx], sigma);
  }
...
generated quantities {
  ...
  for(tIdx in 1 : N){
    ...
    timeWaitedLogProb[nTimePoints[tIdx]+1, tIdx] = sum(waitLogProb[1:nTimePoints[tIdx], tIdx]);
    log_lik[tIdx] =  log_lik[tIdx] + sum(waitLogProb[1:nTimePoints[tIdx], tIdx]) + normal_lpdf(nTimePoints[tIdx] * stepDuration | timeWaited[tIdx], sigma);
  }
